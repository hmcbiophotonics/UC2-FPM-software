...
std::string SETUP = "FPMSCOPE";      //S006->Aurelie; S004->Barbora
std::string COMPONENT = "ZSTAGE"; // LAR01 //LED01 // MOT02=x,y // MOT01=z
...
Stepper stepperZ = Stepper(2038,25,26,27,14); // steps/rev., then pin nums
// ~~~~ FLUO ~~~~
...
const char *COMMANDSET[NCOMMANDS] = {"DRVX", "DRVY", "DRVZ", "FLUO", "SETSPEED"};
...
void callback(char *topic, byte *message, unsigned int length)
{
    ...
    if (std::string(topic) == stopicREC)
    {
        ...
        if (strcmp(CMD, COMMANDSET[0]) == 0)
        {
          ...
          Serial.println("unimplemented: stepper X move");
          
        }
        else if (strcmp(CMD, COMMANDSET[1]) == 0)
        {
          //stepperY.Move((int)(INSTS[0] * 10));
          Serial.println("unimplemented: stepper Y move");
        }
        else if (strcmp(CMD, COMMANDSET[2]) == 0)
        {
            stepperZ.step(INSTS[0] *  10);
        }
        ...
        {
          stepperZ.setSpeed(INSTS[0]);
          Serial.println("set Z speed to");
